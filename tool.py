...
PORT = 12343
TENCENTCLOUD_REGISTRY = "hkccr.ccs.tencentyun.com"
MAX_REPO_SIZE = 500
...
class Toolkit:
    ...
    def check_aicrowd_json(self):
        ...
        if config.get(
            ...
            err(f"[challenge_id] in aicrowd.json should be neurips-2022-the-neural-mmo-challenge"
                )
            sys.exit(3)
    ...
    def submit(
        ...
        warn(
            f"Please make sure putting all your submission related (code, model, ...) in the my-submission folder."
        )
